...
function clusterize2(params) {
    if (arguments.length > 1) 
        return clusterize.apply(null, Array.prototype.slice(arguments));
...
function clusterize(workerFn, masterFn, reforkOnDeath, workersToCoresRatio, workersCount, workerDeathCallback, env) {
    ...
    if (cluster.isMaster) {        
        ...
        cluster.on('online', function(worker) {
            ...
            console.log('worker spawned at %s', address);
        });
        ...
        cluster.on('exit', function(worker, code, signal) {
            clearTimeout(timeouts[worker.id]);
            console.log('worker exit. code: [%s], signal:[%s]', code, signal);
        });
